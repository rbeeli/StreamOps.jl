mutable struct LiveTimerSource{TPeriod,TTime,TSourceFunc}
    node::StreamNode
    ...
    function LiveTimerSource{TTime}(executor, node::StreamNode; interval::TPeriod, start_time::TTime) where {TPeriod,TTime}
...
function worker(timer::LiveTimerSource{TPeriod,TTime}, executor::RealtimeExecutor{TStates,TTime}) where {TPeriod,TStates,TTime}
    try
        while true
        ...
        println("LiveTimerSource: Timer [$(timer.node.label)] task ended")
    catch e
        println("LiveTimerSource: Timer [$(timer.node.label)] task ended with error: $e")
    end
end

function run!(timer::LiveTimerSource{TPeriod,TTime}, executor::RealtimeExecutor{TStates,TTime}) where {TPeriod,TStates,TTime}
    # i = 0
...
function process_event!(adapter::LiveTimerSource{TPeriod,TTime}, executor::RealtimeExecutor{TStates,TTime}, event::ExecutionEvent{TTime}) where {TPeriod,TStates,TTime}
...
function destroy!(timer::LiveTimerSource{TPeriod,TTime}) where {TPeriod,TTime}
