...
mutable struct TimeCount{TTime,TPeriod,interval_mode} <: StreamOperation
    ...
    function TimeCount{TTime}(time_period::TPeriod, interval_mode::Symbol) where {TTime,TPeriod}
        new{TTime,TPeriod,interval_mode}(Vector{TTime}(), time_period, interval_mode, 0)
    end
end
...
@inline function update_time!(
    op::TimeCount{TTime,TPeriod,:open}, current_time::TTime
) where {TTime,TPeriod}
...
@inline function update_time!(
    op::TimeCount{TTime,TPeriod,:closed}, current_time::TTime
) where {TTime,TPeriod}
    cutoff_time = current_time - op.time_period
    removed = 0
    while !isempty(op.time_buffer) && first(op.time_buffer) < cutoff_time
...
@inline function (op::TimeCount{TTime,TPeriod,interval_mode})(
    executor, _
...
@inline function get_state(
    op::TimeCount{TTime,TPeriod,interval_mode}
