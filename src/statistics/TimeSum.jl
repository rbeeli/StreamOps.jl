...
mutable struct TimeSum{TTime,TValue,TPeriod,interval_mode} <: StreamOperation
    ...
    function TimeSum{TTime,TValue}(
        time_period::TPeriod, interval_mode::Symbol
    ) where {TTime,TValue,TPeriod}
        new{TTime,TValue,TPeriod,interval_mode}(
            Vector{TTime}(), Vector{TValue}(), time_period, interval_mode, zero(TValue)
        )
    end
end
...
@inline function update_time!(
    op::TimeSum{TTime,TValue,TPeriod,:open}, current_time::TTime
) where {TTime,TValue,TPeriod}
...
@inline function update_time!(
    op::TimeSum{TTime,TValue,TPeriod,:closed}, current_time::TTime
) where {TTime,TValue,TPeriod}
    cutoff_time = current_time - op.time_period
    while !isempty(op.time_buffer) && first(op.time_buffer) < cutoff_time
        op.current_sum -= popfirst!(op.value_buffer)
...
@inline function (op::TimeSum{TTime,TValue,TPeriod,interval_mode})(
    executor, value
) where {TTime,TValue,TPeriod,interval_mode}
